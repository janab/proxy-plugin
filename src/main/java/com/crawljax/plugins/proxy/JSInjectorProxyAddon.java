 ...
 * 
 */
public class JSInjectorProxyAddon extends ProxyPlugin {

	private File jsFile;
	private FileReader jsFileReader;
	 ...
	 */
	public JSInjectorProxyAddon(File file) throws FileNotFoundException {
		jsFile = file;
		jsFileReader = new FileReader(file);
	}
			...
			try {
				...
				if (contentType.contains("html")) {
					...
					if (nodes.getLength() == 0) {
						...
						System.out.println("JSInjectorProxyAddon: HTML received but"
					...
					// head section found: inject the JS
						...
						if (head != null) {
							...
							System.out.println("Injecting " + jsFile.getAbsolutePath()
							        + " into the DOM!");
							head.appendChild(createInjectedJsNode(dom, jsFileReader));
	 ...
	 *             On read write error.
	 */
	public static Element createInjectedJsNode(Document dom, FileReader fileReader)
	        throws IOException {
		// create the node
		Element el = dom.createElement("script");
		el.setAttribute("type", "text/javascript");
		el.setAttribute("xxx", "injected");
		String js = IOUtils.getStringFromReader(fileReader);
